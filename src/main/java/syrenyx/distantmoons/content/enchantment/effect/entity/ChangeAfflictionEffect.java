  ...
  public void apply(@NonNull ServerLevel world, int level, @NonNull EnchantedItemInUse context, @NonNull Entity target, @NonNull Vec3 pos) {
    ...
    switch (this.operation) {
      case ADD -> AfflictionManager.addToAffliction(livingEntity, new AfflictionInstance(
          afflictionEntry.get(),
          this.stage.map(stageBasedValue -> (int) stageBasedValue.calculate(level)).orElse(Affliction.DEFAULT_STAGE),
          this.progression.map(stageBasedValue -> stageBasedValue.calculate(level)).orElse(0.0F)
      ));
